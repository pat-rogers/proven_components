...
--  This generic package provides sets, in which the members are of a discrete
--  type.
...
package Sequential_Discrete_Sets with
  SPARK_Mode,
  Always_Terminates
is
   ...
   function New_Set (Content : Set_Literal) return Set with
     ...
     Post => (for all C in Element => Contains (Content, C) = Member (New_Set'Result, C)) and then
   ...
   function "+" (Left : Set; Right : Element) return Set with
     Post => --  Members of the result: Right, plus any element previously in Left
             (for all C in Element =>
                Member ("+"'Result, C) = (C = Right or else Member (Left, C))) and then
   ...
   function "+" (Left : Element; Right : Set) return Set with
     Post => --  Members of the result: Left, plus any element previously in Right
             (for all C in Element =>
                Member ("+"'Result, C) = (C = Left or else Member (Right, C))) and then
   ...
   function "-" (Left : Set; Right : Element) return Set with
     Post => not Member ("-"'Result, Right) and then
             --  Members of the result: any element previously in Left, except Right
             (for all C in Element =>
                Member ("-"'Result, C) = (C /= Right and then Member (Left, C))) and then
   ...
   function Contains (Within : Set_Literal;  This : Element) return Boolean;
...
private
   ...
   type Membership is array (Element) of Boolean with Component_Size => 1;
